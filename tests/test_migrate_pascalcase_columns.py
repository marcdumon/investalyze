...
def test_migrate_renames_old_columns(tmp_path):
    ...
    con.execute("CREATE TABLE company_profile (Ticker VARCHAR, Src VARCHAR, "
                'city VARCHAR, fullTimeEmployees BIGINT, FetchedOn DATE)')
    con.execute("INSERT INTO company_profile VALUES ('AAA', 'yahoo', 'Cupertino', 100, DATE '2024-01-01')")

    done = migrate(con)

    cols = {c[0] for c in con.execute('DESCRIBE company_profile').fetchall()}
    ...
    # data preserved
    assert con.execute('SELECT City FROM company_profile').fetchone() == ('Cupertino',)


def test_migrate_is_idempotent(tmp_path):
    db = tmp_path / 'test.duckdb'
    con = duckdb.connect(str(db))
    con.execute('CREATE TABLE company_profile (Ticker VARCHAR, Src VARCHAR, city VARCHAR, FetchedOn DATE)')
    migrate(con)
