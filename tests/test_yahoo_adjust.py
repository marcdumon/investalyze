...
def test_no_events_returns_raw_close():
    ...
    out = _calc_adjusted_close(close, zero, zero)
...
def test_dividend_back_adjusts_prior_days():
    ...
    zero = _series(dates, [0.0, 0.0, 0.0])
    out = _calc_adjusted_close(close, divs, zero)
    factor = 1 - 0.5 / 11.0
...
def test_split_back_adjusts_prior_days():
    # 2-for-1 split on day 3 -> prior prices * 1/2
    dates = ['2024-01-01', '2024-01-02', '2024-01-03']
    close = _series(dates, [10.0, 11.0, 6.0])
    zero = _series(dates, [0.0, 0.0, 0.0])
    splits = _series(dates, [0.0, 0.0, 2.0])
    out = _calc_adjusted_close(close, zero, splits)
